...
const RecommendationCards = ({ recommendations }) => {
  const getCardClass = (alreadyInstalled) => {
    ...
    if (alreadyInstalled === "ai_suggested") return "card-ai-suggested"
    return "card-default"
  }
  ...
  const getStatusText = (alreadyInstalled) => {
    ...
    if (alreadyInstalled === "ai_suggested") return "AI Recommended"
    return "Unknown"
  }
  ...
  const getStatusIcon = (alreadyInstalled) => {
    ...
    if (alreadyInstalled === "ai_suggested") return "🤖"
    return "❓"
  }
  ...
  const aiSuggestedAddons = recommendations.filter((item) => item.already_installed === "ai_suggested")
  ...
  return (
    <div className="recommendations-container">
      ...
      {renderSection("🤖 AI Recommended", aiSuggestedAddons, "ai-suggested-section")}
    </div>
  )
}
